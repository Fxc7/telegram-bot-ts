...
export default {
   ...
   execute: async ({ xcoders, m, languages }: Execute) => {
      try {
         ...
         return xcoders.sendPhoto({ source: response }, { caption: '_Affect Photo..._', parse_mode: 'Markdown' });
      } catch (error) {
         return errorMessage(xcoders, languages.error_message.replace('%%', 'Error generate Affect photo'), error);
      }
   }
};